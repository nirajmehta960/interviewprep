...
// Read existing file
let fileContent = fs.readFileSync('src/lib/data/conceptual.ts', 'utf8');

// If there are syntax artifacts from the previous attempt, clean up from 'const java: ConceptualQuestion[]'
// Let's restore the original clean conceptual.ts content if needed, or parse the clean items.
// Let's re-write conceptual.ts cleanly with all original questions + new high-yield questions.

const newJavaQuestions = [
  {
    topicSlug: "java",
  ...
  },
  {
    ...
    detailedExplanation: [
      "**final (Modifier).**",
      "- Variable: Makes a primitive value unmodifiable or reference un-reassignable (though object internal state can still mutate).",
      "- Method: Prevents subclasses from overriding it.",
      "- Class: Prevents inheritance (e.g., String is final for security and immutability).",
      "**finally (Exception Handling).** Executes regardless of whether an exception was thrown or caught. Commonly used for closing streams or sockets before try-with-resources was introduced in Java 7.",
      "**finalize() (GC Cleanup - Deprecated).** Called by Garbage Collector prior to deallocation. It is unpredictable, unreliable, deprecated in Java 9, and removed in modern Java. Never rely on it for cleanup.",
    ],
    example: {
      language: "JAVA",
      code: `public final class ImmutableHolder { // Cannot be extended\n    private final List<String> items = new ArrayList<>(); // Unreassignable reference\n\n    public void process() {\n        try {\n            System.out.println("Processing...");\n        } catch (Exception e) {\n            System.err.println("Error: " + e.getMessage());\n        } finally {\n            // Guaranteed cleanup block\n            System.out.println("Always executes!");\n        }\n    }\n}`,
    },
    interviewTip:
      "When discussing final variables, clarify that final List<String> prevents reassigning the list reference to a new list, but elements can still be added or removed from the list. Mention try-with-resources as the modern replacement for finally resource cleanup.",
    commonTrap:
      "Thinking finally ALWAYS executes. If System.exit(0) is called or JVM crashes/power loss occurs, finally will not run.",
    followUpQuestions: [
  ...
  },
  {
    ...
    detailedExplanation: [
      ...
      "**Fairness Policy.** ReentrantLock accepts a fairness parameter (new ReentrantLock(true)) to grant lock access to the longest-waiting thread, reducing thread starvation.",
      "**Conditions.** synchronized uses single wait/notify per object monitor. ReentrantLock supports multiple Condition objects (lock.newCondition()) for targeted thread signaling (e.g. BoundedBuffer producer/consumer queues).",
    ],
    example: {
      language: "JAVA",
    ...
    interviewTip:
      "Always emphasize putting lock.unlock() in a finally block! If an exception is thrown inside the try block without a finally block, the ReentrantLock will remain locked forever, deadlocking the application.",
    commonTrap:
  ...
  },
  {
    ...
    detailedExplanation: [
      "**Visibility Guarantee.** Without volatile, threads caching a variable in CPU registers/L1/L2 caches might not see updates written by another thread. volatile flushes writes to main memory immediately and invalidates local thread caches on read.",
      "**Instruction Reordering (Happens-Before).** The compiler and CPU reorder instructions for performance. volatile creates a memory barrier: writes to a volatile variable cannot be reordered before preceding writes, and reads cannot be reordered after subsequent reads.",
      "**Lack of Atomicity.** volatile boolean flag is safe for single-writer flag checks. However, compound read-modify-write operations like count++ (read count, increment, write count) are NOT atomic and require AtomicInteger or synchronized.",
    ],
    example: {
      language: "JAVA",
      code: `public class WorkerThread extends Thread {\n    // Guaranteed to be visible across threads instantly\n    private volatile boolean running = true;\n\n    public void run() {\n        while (running) {\n            // Perform background work\n        }\n    }\n\n    public void stopWorker() {\n        this.running = false; // Immediately flushes to main memory\n    }\n}`,
    },
    interviewTip:
      "A classical question is 'Why is count++ not safe with volatile?'. Explain that count++ compiles into 3 separate bytecode instructions (getfield, iadd, putfield). Two threads reading the same initial value will both write back the same incremented value, causing lost updates.",
    commonTrap:
  ...
  },
  {
    ...
    detailedExplanation: [
      "**String (Immutable).** Stored in the String Constant Pool or heap. Any concatenation (s += \"a\") allocates a new String object, making string loops inefficient.",
      "**StringBuilder (Mutable, Unsynchronized).** Modifies an internal dynamic char/byte array in-place without creating new objects. Best choice for single-threaded string construction.",
      "**StringBuffer (Mutable, Synchronized).** Methods are thread-safe (synchronized). Used when multiple threads mutate the same string buffer concurrently.",
    ],
    example: {
      language: "JAVA",
      code: `// Slow & Memory Intensive: Creates 1000 String objects\nString s = "";\nfor (int i = 0; i < 1000; i++) s += i;\n\n// Fast & Efficient (Single-threaded): Modifies internal array in-place\nStringBuilder sb = new StringBuilder();\nfor (int i = 0; i < 1000; i++) sb.append(i);\nString result = sb.toString();`,
    },
    interviewTip:
      "Note that since Java 5, the compiler automatically converts simple \"a\" + \"b\" + \"c\" concatenations into StringBuilder calls. However, inside explicit for loops, manual StringBuilder is mandatory.",
    commonTrap:
      "Using StringBuffer everywhere out of habit. In 99% of single-threaded scenarios, StringBuilder is preferred due to zero synchronization overhead.",
    followUpQuestions: [
  ...
  },
  {
    ...
    detailedExplanation: [
      "**Checked Exceptions.** Represent recoverable conditions outside application control (e.g., IOException, SQLException). The compiler forces the caller to explicitly handle or declare them.",
      "**Unchecked Exceptions (RuntimeException).** Represent programming flaws or invalid API usage (e.g., NullPointerException, ArrayIndexOutOfBoundsException, IllegalArgumentException).",
      "**Errors.** Subclasses of java.lang.Error (e.g. OutOfMemoryError, StackOverflowError) represent catastrophic system-level failures that applications should not attempt to catch.",
    ],
    example: {
      language: "JAVA",
      code: `// Checked Exception: Mandatory handling or throws clause\npublic void readFile(String path) throws IOException {\n    FileReader file = new FileReader(path); // Must handle IOException\n}\n\n// Unchecked Exception: Runtime logic error, no mandatory throws\npublic void processName(String name) {\n    if (name == null) {\n        throw new IllegalArgumentException("Name cannot be null");\n    }\n}`,
    },
    interviewTip:
...
const newPythonQuestions = [
  {
    ...
    detailedExplanation: [
      "**Identity ('is').** Compares object pointers. a is b is True if and only if id(a) == id(b). It is fast and evaluates memory addresses.",
      ...
      "**Integer & String Interning Trap.** CPython caches small integers (-5 to 256) and short string literals. Thus 256 is 256 is True, but 1000 is 1000 may be False depending on context. Always use == for value comparisons!",
    ],
    example: {
      language: "PYTHON",
      code: `a = [1, 2, 3]\nb = [1, 2, 3]\nc = a\n\na == b  # True  — values are identical\na is b  # False — distinct objects in memory\na is c  # True  — point to same object\n\n# CPython Interning Trap\nx = 256\ny = 256\nx is y  # True (small integer cached)\n\nm = 1000\nn = 1000\nm is n  # False (separate heap allocations)`,
    },
    interviewTip:
  ...
  },
  {
    ...
    example: {
      language: "PYTHON",
      code: `x = "Global"\n\ndef outer():\n    x = "Enclosing"\n    def inner():\n        nonlocal x\n        x = "Modified Enclosing"\n        print("Local:", x)\n    inner()\n    print("Outer:", x)\n\nouter()\n# Output:\n# Local: Modified Enclosing\n# Outer: Modified Enclosing`,
    },
    interviewTip:
      "Shadowing built-ins is a common interview trap: if you name a variable list = [1, 2] or sum = 10, you shadow the built-in functions list() and sum(), causing unexpected TypeError: 'list' object is not callable later.",
    commonTrap:
  ...
  },
  {
    ...
    interviewTip:
      "Mention that context managers are useful for far more than opening files: acquiring DB connections, setting temporary environment configs/decimal precision, timing code blocks, and acquiring threading locks (with lock:).",
    commonTrap:
  ...
  },
  {
    ...
    example: {
      language: "PYTHON",
      code: `class Celsius:\n    def __init__(self, temp=0):\n        self._temp = temp\n\n    @property\n    def temp(self):\n        return self._temp\n\n    @temp.setter\n    def temp(self, val):\n        if val < -273.15:\n            raise ValueError("Temperature below absolute zero!")\n        self._temp = val\n\nc = Celsius(25)\nc.temp = 30         # Calls setter transparently\nprint(c.temp)       # Calls getter transparently (30)`,
    },
    interviewTip:
  ...
  },
  {
    ...
    detailedExplanation: [
      ...
      "- threading: Ideal for I/O-bound tasks where threads spend time waiting for responses (HTTP requests, DB queries). The GIL is released during system I/O calls.",
      "- multiprocessing: Ideal for CPU-heavy tasks (image processing, math, machine learning). Each process gets its own Python interpreter and GIL.",
      "**Memory & Overhead.** Threads share memory easily (requiring locks to avoid race conditions). Processes have separate memory spaces, requiring IPC (Queues, Pipes, SharedMemory) and higher startup overhead.",
    ],
    example: {
      language: "PYTHON",
      code: `from concurrent.futures import ThreadPoolExecutor, ProcessPoolExecutor\nimport time\n\ndef io_bound_task(url):\n    time.sleep(1) # Simulates network request\n    return "Done"\n\ndef cpu_bound_task(n):\n    return sum(i * i for i in range(n))\n\n# I/O-bound: Threading runs concurrently during sleep\nwith ThreadPoolExecutor(max_workers=4) as executor:\n    results = list(executor.map(io_bound_task, ["url1", "url2", "url3"]))\n\n# CPU-bound: Multiprocessing uses 4 physical cores simultaneously\nwith ProcessPoolExecutor(max_workers=4) as executor:\n    results = list(executor.map(cpu_bound_task, [10**7, 10**7, 10**7]))`,
    },
    interviewTip:
...
// Re-read clean conceptual.ts before our attempt if needed, or cut at boundaries
let cleanContent = fs.readFileSync('src/lib/data/conceptual.ts', 'utf8');

// Find where 'const java: ConceptualQuestion[] = [' begins and where 'const python: ConceptualQuestion[] = [' begins
const javaStartIndex = cleanContent.indexOf('const java: ConceptualQuestion[] = [');
const pythonStartIndex = cleanContent.indexOf('const python: ConceptualQuestion[] = [');
const exportStartIndex = cleanContent.indexOf('export const conceptualQuestions');

// Extract clean original java items (lines from javaStartIndex to before python)
// Let's strip any corrupted text from earlier run if present
let javaBlock = cleanContent.substring(javaStartIndex, pythonStartIndex);
let pythonBlock = cleanContent.substring(pythonStartIndex, exportStartIndex);

// Convert new array objects to formatted JS strings
const formatQuestion = (q) => {
  return `  {\n` +
    `    topicSlug: ${JSON.stringify(q.topicSlug)},\n` +
    ...
    `    synopsis: ${JSON.stringify(q.synopsis)},\n` +
    `    shortAnswer:\n      ${JSON.stringify(q.shortAnswer)},\n` +
    `    detailedExplanation: ${JSON.stringify(q.detailedExplanation, null, 6)},\n` +
    (q.example ? `    example: {\n      language: ${JSON.stringify(q.example.language)},\n      code: ${JSON.stringify(q.example.code)},\n    },\n` : '') +
    `    interviewTip:\n      ${JSON.stringify(q.interviewTip)},\n` +
    (q.commonTrap ? `    commonTrap:\n      ${JSON.stringify(q.commonTrap)},\n` : '') +
    `    followUpQuestions: ${JSON.stringify(q.followUpQuestions, null, 6)},\n` +
    `    relatedTopics: ${JSON.stringify(q.relatedTopics, null, 6)},\n` +
    `    tags: ${JSON.stringify(q.tags, null, 6)},\n` +
    `  }`;
};

const formattedJavaNew = newJavaQuestions.map(formatQuestion).join(',\n');
const formattedPythonNew = newPythonQuestions.map(formatQuestion).join(',\n');

// Clean up java block end array
let javaArrayInner = javaBlock.trim();
if (javaArrayInner.endsWith('];')) {
  javaArrayInner = javaArrayInner.substring(0, javaArrayInner.lastIndexOf('];')).trim();
}

let pythonArrayInner = pythonBlock.trim();
if (pythonArrayInner.endsWith('];')) {
  pythonArrayInner = pythonArrayInner.substring(0, pythonArrayInner.lastIndexOf('];')).trim();
}

const finalFile = cleanContent.substring(0, javaStartIndex) +
  javaArrayInner + ',\n' + formattedJavaNew + '\n];\n\n' +
  pythonArrayInner + ',\n' + formattedPythonNew + '\n];\n\n' +
...
fs.writeFileSync('src/lib/data/conceptual.ts', finalFile);
console.log('Successfully wrote updated conceptual.ts');
