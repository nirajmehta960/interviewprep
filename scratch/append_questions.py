...
java_new_questions = """,
  {
    ...
    detailedExplanation: [
      ...
      "**Eager vs Lazy.** Collections are eagerly evaluated: elements exist in memory before you interact with them. Streams are lazy: intermediate operations (like \`filter\` or \`map\`) build a execution pipeline and perform zero work until a terminal operation (like \`collect\` or \`findFirst\`) is invoked.",
...
public void processName(String name) {
    ...
    interviewTip:
      "Modern frameworks (like Spring) favor Unchecked Exceptions to reduce boilerplate \`throws\` declarations while wrapping infrastructure errors in clean runtime runtime exceptions.",
    commonTrap:
...
with open("src/lib/data/conceptual.ts", "r") as f:
    content = f.read()

# Insert java_new_questions before 'const python: ConceptualQuestion[]'
content = content.replace("const python: ConceptualQuestion[] = [", java_new_questions + "\n];\n\n/* ==========================================================================\n   Python\n   ========================================================================== */\n\nconst python: ConceptualQuestion[] = [")

# Insert python_new_questions before 'export const conceptualQuestions'
content = content.replace("export const conceptualQuestions: ConceptualQuestion[] = [...java, ...python];", python_new_questions + "\n];\n\n/** Every authored conceptual question, in stable per-topic order. */\nexport const conceptualQuestions: ConceptualQuestion[] = [...java, ...python];")

with open("src/lib/data/conceptual.ts", "w") as f:
    f.write(content)

print("Successfully added 13 high-yield theoretical questions to conceptual.ts!")
